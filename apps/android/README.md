...
手机登录默认持续 12 小时，由云端 `auth.approver_token_ttl_ms` 配置，过期重新登录。审批页显示最近成功同步时间，30 秒不同步即禁用审批。通知权限被拒绝后仍可从登录页或审批页开启提醒，未开启时请保持 App 打开手动审批。

单元测试覆盖 HTTPS 地址边界和申请到期禁用；Robolectric Service / Activity 测试覆盖实际定时轮询、网络恢复、通知异常、权限恢复、登录过期、同步超时、任务异常停止和退出时的迟到响应。Robolectric 仅是测试依赖，不进入 APK；lint 检查最低系统版本、权限与资源。云端 HTTP/WSS 授权、并发与自动连接由 C++ 测试覆盖。上线前还需要验证目标手机安装、通知、后台值守与真实云端证书。
