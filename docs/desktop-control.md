...
- 三个平台均提供原生“编辑”菜单，支持撤销、重做、剪切、复制、粘贴和全选。macOS 使用 Cmd 快捷键，Windows/Ubuntu 使用 Ctrl；登录输入框保留原生文本编辑行为。
...
Windows 使用 `--platform win32 --arch x64` 或 `--arch arm64`；Linux 使用 `--platform linux --arch x64` 或 `arm64`。平台和架构必须与原生二进制头一致，打包器会校验。默认配置来自 `configs/driver-console.three-machine.dev.yaml`，需要另一套预置时显式传 `--config /path/driver-console.yaml`，并把对应证书放入原生包 config 目录。
...
- Windows：`scripts/build/build_windows_control_bundle.ps1` 使用已有锁定 MSVC/vcpkg 构建；CMake `InstallRequiredSystemLibraries` 将编译器运行库复制到 EXE 旁，打包时检查 `vcruntime140.dll` 和 `msvcp140.dll`。CI 使用原生包封装 Chromium 桌面 ZIP，并执行 Node 生命周期测试。三平台矩阵与发布规则见 [CI 与自动发布](ci-release.md)。见 [CMake 运行库分发说明](https://cmake.org/cmake/help/latest/module/InstallRequiredSystemLibraries.html)。
...
- Ubuntu 22.04 arm64 原生账号回归通过，打包后 113 个 ELF 依赖解析检查通过，包内服务生命周期测试 9/9；Windows x64 原生代码 MinGW 交叉编译通过。Windows x64 的 MSVC 编译、原生生命周期测试、Chromium 桌面打包及上传已在 PR #21 的 CI 中通过；它仍不替代目标桌面和实车验收。
