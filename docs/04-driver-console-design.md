...
- 急刹优先于缓刹；任一刹车输入存在时油门输出为 0，转向输入保持独立。
- 前进/倒车键选择的 `D`/`R` 在当前控制会话内锁存；松开按键只将油门归零，普通刹车不得把挡位切回 `N`。
- `D` 与 `R` 互切只允许发生在当前指令挡位为 `N`、明确不支持 VCU 握手的 mock，或 VCU 给出有效且绝对值不大于 `0.1 m/s` 的零速反馈时；方向键与锁存挡位不匹配时油门必须保持 0，并提示驾驶员停车后释放、重新按键。
- 多个物理键映射到同一方向时必须按键码集合派生状态；例如同时按住 `ArrowUp` 和 `W`，释放其中一个不能清除前进状态。
- 首次获得 VCU Ready 前不得记录运动按键；只有当前会话曾经 Ready 后的 `wait_gear` / `wait_actuator_modes` 闭环等待才保留按键集合，并继续按 20 Hz 发送油门强制为 0 的完整安全快照。
...
合成为 `steering`、`throttle`、单一 v1 `brake`、`estop` 和 `gear`。缓刹与急刹
