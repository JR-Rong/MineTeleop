...
本仓库按 `JYR010_DBC_VCU_20260916.dbc` 实现车端 VCU 通讯。新版 bridge
已用固定 CAN 报文和状态机测试验证协议适配；这些测试不代表实车握手、执行器
响应、制动效果或总线负载验收。历史 20260714 文件仅留作协议对照。
...
本仓库基于 20260916 协议实现的 CAN 编解码和 EPB 枚举，不能照搬旧代码中不一致
的 EPB 反馈值。
...
新版 DBC 的八路 MCU 转矩请求与实际转矩反馈均为 **16-bit、0.1 Nm/bit、
偏移 -3200 Nm**，线编码可表示 `[-3200, 3353.5] Nm`。请求从 bit 8 开始，
反馈从 bit 16 开始；转速字段的位置与编码不变。零转矩为 raw `32000`，小端
字节 `00 7D`；旧编码 `40 1F` 在新版协议下会被解释为 `-2400 Nm`，因此不能
将此 bridge 与旧协议 VCU 混用，也不进行自动协议猜测。

普通驾驶 D/R 对称上限仍为 `640.0 Nm/单电机`，bridge 缺省值仍为 `300 Nm`；
现场车端模板仍为 `640 Nm`，控制页初始值取车辆上报上限的一半。本次仅改协议
编码，不随线编码量程扩大输出限制。

文档存在两处待供应方澄清的不一致：新版 DBC 的 MCU02 请求 min/max 元数据
仍为旧范围，但位宽、比例、偏移已更新；同名 20260916 Excel 的部分转矩定义
仍是 14-bit/-800。本实现以 DBC 的位宽、比例、偏移为准，640 Nm 上限也落在
MCU02 遗留的旧声明范围内。提高输出上限前必须先核准协议并完成台架标定。

回归覆盖八路电机的零转矩、正反向 640 Nm 固定字节、相邻转速字段不被覆盖，
以及实采 `40 9F 00 7D 00 7D 2F 36` 零转矩反馈允许退出扭矩等待。
握手、制动模式和驻车状态机在此版本中保持原有逻辑；制动模式仍回报 0 等独立
现场问题，需要通过新版本车端日志和实车反馈另行确认。
