...
控制 DataChannel 默认采用 unordered/unreliable 配置。控制命令以 20 Hz 发送完整状态，并通过 `seq` 去重；可靠有序模式的重传和队头阻塞不适合作为默认遥操控制链路。
...
- 验证 WebRTC H.264 SDP 协商和编码 profile/level，确保 VAAPI 输出与驾驶端解码器兼容。
- 建立弱网测试基线。
