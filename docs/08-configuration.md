...
触发本地安全停车，同时只重开故障 lane；该相机后续重新变新鲜后，内部的控制锁存
会自动解除，但本次故障关闭的控制 DataChannel 不会自己重新打开，驾驶端仍需重新
建立控制连接才能真正拿回控制权；若该关键相机的重开额度耗尽而被禁用，控制锁存
不会再自动解除，必须结束当前 session 并在新 session 中重新完成 VCU 握手。若同一
路配置的 V4L2 设备路径在打开前就完全不存在，首次尝试即视为不可重试直接禁用该
lane，不消耗/等待重开退避周期。非关键相机在重开额度耗尽后只禁用自身 lane，
其他视频和当前控制不受影响。
