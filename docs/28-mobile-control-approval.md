...
auth:
  ...
  approver_token_ttl_ms: 43200000
  drivers:
...
- 手机登录有效期使用 `auth.approver_token_ttl_ms`，默认 12 小时，范围 1 毫秒至 7 天，与驾驶员 `--driver-token-ttl-ms` 分离。到期停止值守并重新登录，不自动续期。返回的 `remaining_ms` 用于手机单调时钟倒计时；旧云端只返回绝对到期时间时仍兼容。
- 审批密码错误锁定按可信来源 IP 隔离，不会锁死其他来源或使已登录手机失效。来源解析复用 `trusted_proxy_addresses`；共享出口的手机仍共用一个来源桶。默认 60 秒内 5 次错误锁定该来源 5 分钟。
- 失败来源记录复用 `api_rate_limit_max_sources` 容量（默认 4096）；超出容量的未记录来源共享溢出桶，已有来源保持独立。到期记录会定期清理；原有 API 来源限流继续生效。
...
原生 Android 项目位于 `apps/android/`，支持 Android 8.0（API 26）及以上。构建与安装见 `apps/android/README.md`。
...
值守期间由用户主动启动的 Android 前台服务每 3 秒拉取申请，显示常驻值守通知；新申请产生系统通知，点击进入 App 核对后决定。未开启通知时可保持 App 打开手动审批，但无法后台提醒；登录页和审批页均可重新请求权限或进入通知设置，恢复权限后仍待处理的未提醒申请会补发通知。本版无 Firebase / 厂商推送依赖；休眠、省电策略、断网及强行停止 App 仍可能延迟或阻止提醒，需在目标手机验证。App 被系统杀死或登录过期后须重新输入密码，不自动恢复授权值守。

审批页显示最近成功同步距今的秒数。30 秒未成功同步时禁用审批并显示离线状态，恢复后重新开放。通知发送 / 取消异常不会终止网络轮询；独立状态检查发现定时任务异常结束时停止值守，要求重新登录。系统休眠时本地检查本身也可能延迟，仍需真机验证。
...
原生客户端统一放在 `apps/` 下，当前只有 `apps/android/`；以后可并列新增 `apps/ios/` 或其他平台。云端只提供 `/mobile/api/*` 审批接口，不再提供 `/mobile/` 网页审批入口。
...
8. 已建立会话仍使用原来的控制租约、续期与安全退出机制；同一会话续期无需重新审批。会话关闭后再次申请建立新会话需要重新审批，不设置免审批重连窗口；现场需验证实际断线是否导致会话关闭。

| 接口 | 身份 | 用途 |
|---|---|---|
| `POST /mobile/api/login` | `password` | 返回审批专用 `token`、`expires_at_utc_ms` 和 `remaining_ms` |
...
`mine-teleop-mobile-approval-tests` 覆盖默认放行、无批准拦截、请求去重、一次性消费、竞争决定、拒绝/超时、身份用途隔离、审批车辆范围、连接代次变更、吊销、审计失败、来源隔离 / 可信代理 / 溢出桶 / 限流恢复、独立 App 登录期限、已删除网页路由返回 404，以及控制端一次调用自动建立 HTTP/WSS 会话、拒绝/超时/退出结束等待和迟到授权响应的清理。软件测试不等同于现场验收；部署后还需用实际两部手机、目标控制端和安全停稳的车辆验证 HTTPS、后台行为、在线审批与断线恢复。
