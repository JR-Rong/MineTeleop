...
runtime。构建完成后会在 Ubuntu 22.04 amd64 容器中运行包级自检。
...
交叉构建且当前 Mac 无法执行目标架构程序时，可以显式生成 build-only 包：

```bash
MINE_TELEOP_MACOS_ARCH=x64 \
MINE_TELEOP_SKIP_RUN_TESTS=1 \
  scripts/build/build_macos_control_bundle.sh
```
...
5. 在 amd64 容器中运行 libsrtp CTest 和 Mine Teleop CTest；
6. 收集动态库、GStreamer 插件、CA 和相机辅助文件；
7. 对最终 `.tar.gz` 运行干净 Ubuntu 容器验收。
...
该模式仍会重新构建当前 Mine Teleop 二进制并重新验证最终安装包。
