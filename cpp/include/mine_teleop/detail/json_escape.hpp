...
inline std::size_t valid_utf8_sequence_length(
  ...
  if (first == 0xe0U) {
    return remaining >= 3U && at(1U) >= 0xa0U && at(1U) <= 0xbfU &&
                   is_utf8_continuation(at(2U))
               ? 3U
               : 0U;
  }
  if ((first >= 0xe1U && first <= 0xecU) || (first >= 0xeeU && first <= 0xefU)) {
    return remaining >= 3U && is_utf8_continuation(at(1U)) &&
                   is_utf8_continuation(at(2U))
               ? 3U
               : 0U;
  }
  if (first == 0xedU) {
    return remaining >= 3U && at(1U) >= 0x80U && at(1U) <= 0x9fU &&
                   is_utf8_continuation(at(2U))
               ? 3U
               : 0U;
  }
  if (first == 0xf0U) {
    return remaining >= 4U && at(1U) >= 0x90U && at(1U) <= 0xbfU &&
                   is_utf8_continuation(at(2U)) && is_utf8_continuation(at(3U))
               ? 4U
               : 0U;
  }
  if (first >= 0xf1U && first <= 0xf3U) {
    return remaining >= 4U && is_utf8_continuation(at(1U)) &&
                   is_utf8_continuation(at(2U)) && is_utf8_continuation(at(3U))
               ? 4U
               : 0U;
  }
  if (first == 0xf4U) {
    return remaining >= 4U && at(1U) >= 0x80U && at(1U) <= 0x8fU &&
                   is_utf8_continuation(at(2U)) && is_utf8_continuation(at(3U))
               ? 4U
               : 0U;
  }
      ...
      default:
        if (character < 0x20U) {
          ...
          const auto sequence_length =
              json_escape_detail::valid_utf8_sequence_length(value, index);
