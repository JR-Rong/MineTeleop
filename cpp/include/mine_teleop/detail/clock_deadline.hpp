...
constexpr std::int64_t saturating_deadline_ms(std::int64_t start_ms, std::int64_t duration_ms) {
  if (duration_ms <= 0) return start_ms;
...
constexpr MonotonicMillis local_monotonic_deadline_from_utc_expiry(
    UtcMillis expires_at_utc,
    ClockSample received_at) {
  if (expires_at_utc.value <= received_at.utc.value) return received_at.monotonic;
  return MonotonicMillis{saturating_deadline_ms(
      received_at.monotonic.value,
