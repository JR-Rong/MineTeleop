...
.label{position:absolute;left:8px;top:8px;background:#000a;padding:4px 8px;border-radius:4px;z-index:2}pre{background:#030712;padding:12px;overflow:auto}.keys,.muted{color:#9ca3af}.auth{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin:12px 0}[hidden]{display:none!important}dialog{color:#e5e7eb;background:#111827;border:1px solid #4b5563;border-radius:10px;max-width:520px;width:calc(100% - 40px)}dialog::backdrop{background:#000a}.limit-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.limit-grid label{display:flex;flex-direction:column}.limit-warning{border-left:4px solid #f59e0b;padding:8px 10px;background:#1f2937}
...
<section id="session-panel" class="auth" hidden><label for="vehicle">授权车辆</label><select id="vehicle"></select><button id="connect">连接所选车辆</button><button id="control-limits-open">实车调试限幅</button><strong id="control-limits-summary" class="warn">油门 ≤5% · 转向 ≤3.0°</strong><button id="logout">安全退出</button><button id="estop" class="danger">急停</button><strong id="webrtc">未连接</strong><span id="auth-expiry" class="muted"></span></section>
...
const controlLimits={maxThrottle:limitConfig.initial_max_throttle,maxSteeringDeg:limitConfig.initial_max_steering_angle_deg};
let vehicleHardLimits={max_throttle:1,max_steering_angle_deg:limitConfig.steering_full_scale_deg,max_speed_kph:null,received:false};
...
const webrtcLabel=document.getElementById('webrtc'),cameraGrid=document.getElementById('cameras'),statusPanel=document.getElementById('status'),loginPanel=document.getElementById('login-panel'),sessionPanel=document.getElementById('session-panel'),passwordInput=document.getElementById('password'),vehicleSelect=document.getElementById('vehicle'),connectButton=document.getElementById('connect'),authExpiry=document.getElementById('auth-expiry'),gamepadPanel=document.getElementById('gamepad-panel'),gamepadStatus=document.getElementById('gamepad-status'),gamepadValues=document.getElementById('gamepad-values'),vcuPanel=document.getElementById('vcu-panel'),vcuStatus=document.getElementById('vcu-status'),vcuGate=document.getElementById('vcu-gate'),vcuConnectButton=document.getElementById('vcu-connect'),vcuDisconnectButton=document.getElementById('vcu-disconnect'),controlLimitsOpen=document.getElementById('control-limits-open'),controlLimitsSummary=document.getElementById('control-limits-summary'),controlLimitsDialog=document.getElementById('control-limits-dialog'),maxThrottlePercent=document.getElementById('max-throttle-percent'),maxSteeringDeg=document.getElementById('max-steering-deg'),vehicleHardLimitsLabel=document.getElementById('vehicle-hard-limits'),controlLimitsConfirm=document.getElementById('control-limits-confirm'),controlLimitsApply=document.getElementById('control-limits-apply'),controlLimitsCancel=document.getElementById('control-limits-cancel'),estopStatus=document.getElementById('estop-status'),monitorPanel=document.getElementById('monitor-panel'),alertsPanel=document.getElementById('alerts'),streamMetrics=document.getElementById('stream-metrics');
let peer=null,controlChannel=null,pendingIce=[],remoteCameraIds=[],offeredCameraByMid=new Map(),iceServers=[],polling=false,connecting=false,authenticated=false,heartbeatInFlight=false,mediaStatus={lanes:[]},h265FailureSamples=0,h265FallbackSent=false,estopLatched=false,gamepadEstopPressedAt=0,activeGamepadIndex=null,latestMetrics={streams:[]},latestRuntimeStatus={},lastAlertKey='',controlAuthorityLost=false,signalingGeneration=0,signalingPollAbort=null,vcuHandshake={supported:false,state:'unavailable',ready:false,requested:false,disarming:false,parking_ready:false,driver_connected:false};const previousStats=new Map(),cameraByMid=new Map(),assignedCameraIds=new Set();
...
function effectiveControlLimits(){return{maxThrottle:Math.min(controlLimits.maxThrottle,vehicleHardLimits.max_throttle),maxSteeringDeg:Math.min(controlLimits.maxSteeringDeg,vehicleHardLimits.max_steering_angle_deg)}}
function renderControlLimits(){const effective=effectiveControlLimits();controlLimitsSummary.textContent=`油门 ≤${(effective.maxThrottle*100).toFixed(0)}% · 转向 ≤${effective.maxSteeringDeg.toFixed(1)}°`;controlLimitsSummary.className=effective.maxThrottle<=0.1&&effective.maxSteeringDeg<=5?'ok':'warn';maxThrottlePercent.max=String(vehicleHardLimits.max_throttle*100);maxSteeringDeg.max=String(vehicleHardLimits.max_steering_angle_deg);vehicleHardLimitsLabel.textContent=vehicleHardLimits.received?`车端硬上限：油门 ${(vehicleHardLimits.max_throttle*100).toFixed(0)}% · 转向 ${vehicleHardLimits.max_steering_angle_deg.toFixed(1)}° · 配置速度 ${Number(vehicleHardLimits.max_speed_kph).toFixed(1)} km/h`:'等待车端硬上限；应用值仍会被车端再次截断'}
function updateVehicleHardLimits(value){if(!value||typeof value!=='object')return;const throttle=Number(value.max_throttle),steering=Number(value.max_steering_angle_deg),speed=Number(value.max_speed_kph);if(!Number.isFinite(throttle)||throttle<0||throttle>1||!Number.isFinite(steering)||steering<0||steering>limitConfig.steering_full_scale_deg||!Number.isFinite(speed)||speed<0)return;vehicleHardLimits={max_throttle:throttle,max_steering_angle_deg:steering,max_speed_kph:speed,received:true};const previous={...controlLimits};controlLimits.maxThrottle=Math.min(controlLimits.maxThrottle,throttle);controlLimits.maxSteeringDeg=Math.min(controlLimits.maxSteeringDeg,steering);if(previous.maxThrottle!==controlLimits.maxThrottle||previous.maxSteeringDeg!==controlLimits.maxSteeringDeg){clearControlInput();clientLog('control_limits_reduced_by_vehicle',{requested_max_throttle:previous.maxThrottle,requested_max_steering_angle_deg:previous.maxSteeringDeg,effective_max_throttle:controlLimits.maxThrottle,effective_max_steering_angle_deg:controlLimits.maxSteeringDeg})}renderControlLimits()}
function openControlLimits(){maxThrottlePercent.value=(controlLimits.maxThrottle*100).toFixed(0);maxSteeringDeg.value=controlLimits.maxSteeringDeg.toFixed(1);controlLimitsConfirm.checked=false;controlLimitsApply.disabled=true;renderControlLimits();controlLimitsDialog.showModal()}
function applyControlLimits(){if(!controlLimitsConfirm.checked)throw Error('请先确认隔离台架条件');if(vcuHandshake.supported&&(!vcuHandshake.speed_valid||Math.abs(Number(vcuHandshake.speed_mps))>0.1))throw Error('VCU 车速未确认归零，禁止修改实车限幅');const throttlePercent=Number(maxThrottlePercent.value),steeringDeg=Number(maxSteeringDeg.value);if(!Number.isFinite(throttlePercent)||throttlePercent<0||throttlePercent>vehicleHardLimits.max_throttle*100)throw Error('油门上限超出车端硬上限');if(!Number.isFinite(steeringDeg)||steeringDeg<0||steeringDeg>vehicleHardLimits.max_steering_angle_deg)throw Error('转向上限超出车端硬上限');clearControlInput();controlLimits.maxThrottle=throttlePercent/100;controlLimits.maxSteeringDeg=steeringDeg;controlLimitsDialog.close();renderControlLimits();clientLog('driver_control_limits_applied',{max_throttle:controlLimits.maxThrottle,max_steering_angle_deg:controlLimits.maxSteeringDeg,vehicle_hard_limits:vehicleHardLimits});statusPanel.textContent=`调试限幅已应用：油门 ${throttlePercent.toFixed(0)}%，转向 ${steeringDeg.toFixed(1)}°`;send({},false).catch(console.error)}
...
function closeRealtimeSession(){const generation=suspendSignalingPoll();clearControlInput();if(controlChannel)controlChannel.close();if(peer)peer.close();controlChannel=null;peer=null;vcuHandshake={supported:false,state:'unavailable',ready:false,requested:false,disarming:false,parking_ready:false,driver_connected:false};pendingIce=[];remoteCameraIds=[];offeredCameraByMid.clear();cameraByMid.clear();assignedCameraIds.clear();previousStats.clear();cameraGrid.replaceChildren();renderVcuHandshake();return generation}
function renderGamepadValues(){const limits=effectiveControlLimits();gamepadValues.textContent=`原始转向 ${gamepadState.steering.toFixed(2)} · 原始油门 ${gamepadState.throttle.toFixed(2)} · 输出上限 ${limits.maxSteeringDeg.toFixed(1)}°/${(limits.maxThrottle*100).toFixed(0)}% · 制动 ${gamepadState.brake.toFixed(2)}`}
...
function requireLogin(message){closeRealtimeSession();authenticated=false;controlAuthorityLost=false;connectButton.textContent='连接所选车辆';renderAuthExpiry(0);sessionPanel.hidden=true;gamepadPanel.hidden=true;vcuPanel.hidden=true;monitorPanel.hidden=true;loginPanel.hidden=false;statusPanel.textContent=message;clientLog('driver_reauthentication_required',{reason:message})}
...
async function logout(){closeRealtimeSession();controlAuthorityLost=true;webrtcLabel.textContent='正在释放控制权';await post('/api/disconnect',{reason:'driver_safe_logout'});authenticated=false;controlAuthorityLost=false;connectButton.textContent='连接所选车辆';renderAuthExpiry(0);sessionPanel.hidden=true;gamepadPanel.hidden=true;vcuPanel.hidden=true;monitorPanel.hidden=true;loginPanel.hidden=false;webrtcLabel.textContent='未连接';statusPanel.textContent=estopLatched?'已安全退出；车辆急停仍需本地确认复位':'已安全退出';clientLog('driver_safe_logout',{estop_latched:estopLatched})}
...
controlLimitsApply.onclick=()=>{try{applyControlLimits()}catch(error){statusPanel.textContent='限幅设置失败: '+error.message}};
...
async function startFromOffer(offer){
  if(peer)peer.close();
  controlChannel=null;
  vcuHandshake={supported:false,state:'unavailable',ready:false,requested:false,disarming:false,parking_ready:false,driver_connected:false};
  clearControlInput();
  ...
  nextPeer.ondatachannel=e=>{
    ...
    channel.onopen=()=>{
      ...
      vcuHandshake={supported:false,state:'unavailable',ready:false,requested:false,disarming:false,parking_ready:false,driver_connected:true};
    ...
    channel.onmessage=event=>{
      ...
      try{
        ...
        vcuHandshake={...(message.status||{}),driver_connected:Boolean(message.driver_connected)};
        if(message.result==='command_rejected')statusPanel.textContent='VCU 握手请求被拒绝，请检查 P 挡、零速、EPB 驻车和人工状态';
        ...
        renderMonitoring();
      }catch(error){clientLog('vcu_handshake_status_invalid',{error:error.message})}
    };
    channel.onclose=()=>{
