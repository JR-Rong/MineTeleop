...
const keys={ArrowLeft:'left',KeyA:'left',ArrowRight:'right',KeyD:'right',ArrowUp:'up',KeyW:'up',ArrowDown:'down',KeyS:'down',Space:'service_brake',KeyB:'hard_brake'};
const state={left:false,right:false,up:false,down:false,service_brake:false,hard_brake:false};
const pressedControlKeys=new Set(),blockedControlKeys=new Set();
...
function syncControlKeyState(){for(const key of Object.keys(state))state[key]=false;for(const code of pressedControlKeys){const binding=keys[code];if(binding)state[binding]=true}}
function vcuMockUnsupported(value=vcuHandshake){return !value?.supported&&value?.state==='unsupported'}
function vcuAllowsGearChange(requestedGear){if(selectedGear==='N'||selectedGear===requestedGear||vcuMockUnsupported())return true;const speed=Number(vcuHandshake.speed_mps);return Boolean(vcuHandshake.speed_valid)&&Number.isFinite(speed)&&Math.abs(speed)<=0.1}
function updateSelectedGearFromHeldDirections(){if(state.up===state.down){pendingGearRequest=null;return}const requestedGear=state.up?'D':'R';if(vcuAllowsGearChange(requestedGear)){selectedGear=requestedGear;pendingGearRequest=null;return}pendingGearRequest=requestedGear;statusPanel.textContent=`${selectedGear}→${requestedGear} 换挡已阻止：需有效零速反馈；请停车后释放并重新按下方向键`}
function clearControlInput(resetGear=true){for(const code of pressedControlKeys)blockedControlKeys.add(code);pressedControlKeys.clear();syncControlKeyState();gamepadState.steering=0;gamepadState.throttle=0;gamepadState.brake=0;gamepadRequiresNeutral=true;if(resetGear){selectedGear='N';pendingGearRequest=null}renderControlState()}
function vcuStateRequiresFreshInput(value=vcuHandshake){const stateName=String(value?.state||'');return stateName==='fault'||stateName==='closed'||stateName==='disarmed'||stateName.startsWith('disarm_')}
function vcuStateKeepsHeldInput(value=vcuHandshake){return vcuEverReady&&['wait_gear','wait_actuator_modes'].includes(String(value?.state||''))}
function acceptControlStatusMessage(message){const sequence=Number(message?.control_status_seq);if(!Number.isSafeInteger(sequence)||sequence<=lastControlStatusSeq){clientLog('control_status_message_dropped',{event:message?.event||'unknown',control_status_seq:Number.isFinite(sequence)?sequence:null,last_control_status_seq:lastControlStatusSeq});return false}lastControlStatusSeq=sequence;return true}
function resetControlAuthorityInput(){vcuEverReady=false;clearControlInput()}
function updateVcuHandshakeState(value){vcuHandshake=value;if(vcuHandshake.ready||vcuMockUnsupported(vcuHandshake))vcuEverReady=true;if(vcuStateRequiresFreshInput(vcuHandshake)){resetControlAuthorityInput();return}if(vcuEverReady&&!vcuHandshake.ready&&!vcuMockUnsupported(vcuHandshake)&&!vcuStateKeepsHeldInput(vcuHandshake))resetControlAuthorityInput()}
...
function sampleGamepad(){if(!gamepadConfig.enabled||document.hidden||!document.hasFocus()){gamepadState.connected=false;gamepadState.steering=0;gamepadState.throttle=0;gamepadState.brake=0;renderControlState();return}const pad=firstConnectedGamepad();if(!pad){gamepadState.connected=false;gamepadState.steering=0;gamepadState.throttle=0;gamepadState.brake=0;renderControlState();return}gamepadState.connected=true;const standard=pad.mapping==='standard';if(standard){const steering=axisValue(pad,0);let steeringValue=steering===null?0:(steering-calibration.steeringCenter)/calibration.steeringRange;if(gamepadConfig.steering_inverted)steeringValue=-steeringValue;gamepadState.steering=clamp(applyDeadzone(steeringValue),-1,1);gamepadState.throttle=clamp(applyPedalDeadzone(buttonValue(pad,7)),0,1);gamepadState.brake=clamp(applyPedalDeadzone(buttonValue(pad,6)),0,1)}else{const steering=axisValue(pad,gamepadConfig.steering_axis),throttle=axisValue(pad,gamepadConfig.throttle_axis),brake=axisValue(pad,gamepadConfig.brake_axis);if(steering===null||throttle===null||brake===null){gamepadState.steering=0;gamepadState.throttle=0;gamepadState.brake=0;renderControlState();return}let steeringValue=(steering-calibration.steeringCenter)/calibration.steeringRange;if(gamepadConfig.steering_inverted)steeringValue=-steeringValue;gamepadState.steering=clamp(applyDeadzone(steeringValue),-1,1);const throttleDelta=gamepadConfig.throttle_inverted?calibration.throttleRest-throttle:throttle-calibration.throttleRest;const brakeDelta=gamepadConfig.brake_inverted?calibration.brakeRest-brake:brake-calibration.brakeRest;gamepadState.throttle=clamp(applyPedalDeadzone(throttleDelta/calibration.throttleRange),0,1);gamepadState.brake=clamp(applyPedalDeadzone(brakeDelta/calibration.brakeRange),0,1)}const gamepadAuthorityReady=vcuEverReady||vcuMockUnsupported();if(!gamepadAuthorityReady){gamepadRequiresNeutral=true;gamepadState.throttle=0;gamepadState.brake=0}else if(gamepadRequiresNeutral){const pedalsNeutral=gamepadState.throttle===0&&gamepadState.brake===0;gamepadState.throttle=0;gamepadState.brake=0;if(pedalsNeutral)gamepadRequiresNeutral=false}if(gamepadState.throttle>0&&selectedGear==='N')selectedGear='D';const estopPressed=buttonValue(pad,gamepadConfig.estop_button)>=0.5;if(estopPressed){if(!gamepadEstopPressedAt)gamepadEstopPressedAt=performance.now();if(performance.now()-gamepadEstopPressedAt>=consoleConfig.estop_hold_ms)latchEstop('Gamepad')}else gamepadEstopPressedAt=0;renderControlState()}
function currentControl(extra={}){let steering=(state.left||state.right)?(state.left===state.right?0:(state.left?-1:1)):gamepadState.steering,throttle=gamepadState.throttle;const requestedGear=state.up!==state.down?(state.up?'D':'R'):null;if(requestedGear)throttle=requestedGear===selectedGear?1:0;else if(state.up&&state.down)throttle=0;const limits=effectiveControlLimits();let brake=clamp(gamepadState.brake,0,1)*limits.hardBrake;if(state.service_brake)brake=Math.max(brake,limits.serviceBrake);if(state.hard_brake)brake=limits.hardBrake;const brakeRequested=state.service_brake||state.hard_brake||gamepadState.brake>0;if(brakeRequested)throttle=0;return{gear:selectedGear,steering:clamp(steering,-1,1)*(limits.maxSteeringDeg/limitConfig.steering_full_scale_deg),throttle:clamp(throttle,0,1)*limits.maxThrottle,brake:clamp(brake,0,limits.hardBrake),estop:estopLatched||Boolean(extra.estop)}}
...
async function writeControl(extra,announceUnavailable){
  const activePeer=peer,activeChannel=controlChannel;
  ...
  const outgoingSnapshot=JSON.stringify(outgoing);
  let prepared;
  ...
  if(peer!==activePeer||controlChannel!==activeChannel||activePeer.connectionState!=='connected'||activeChannel.readyState!=='open'){
    resetControlAuthorityInput();
  ...
  if(!outgoing.estop&&JSON.stringify(latestOutgoing)!==outgoingSnapshot)return{sent:false,reason:'control_intent_superseded'};
  activeChannel.send(JSON.stringify(prepared.command));
...
addEventListener('keydown',e=>{const binding=keys[e.code],estopKey=e.code==='KeyE';if(!binding&&!estopKey)return;if(editingTarget(e.target))return;e.preventDefault();if(!polling){if(binding)blockedControlKeys.add(e.code);lastKeyboardEvent.textContent=`${estopKey?'急停':keyNames[binding]}已截获 · 等待连接`;return}if(estopKey){if(!e.repeat){lastKeyboardEvent.textContent='急停请求已锁定 · E';latchEstop('键盘 E');send({estop:true}).catch(console.error)}return}if(blockedControlKeys.has(e.code)){lastKeyboardEvent.textContent=`${keyNames[binding]}需释放后重新按下 · ${e.code}`;return}if(vcuStateRequiresFreshInput(vcuHandshake)){blockedControlKeys.add(e.code);lastKeyboardEvent.textContent=`${keyNames[binding]}已阻止 · 等待 VCU 恢复后重新按下`;return}if(!vcuEverReady&&!vcuMockUnsupported()){blockedControlKeys.add(e.code);lastKeyboardEvent.textContent=`${keyNames[binding]}已阻止 · 首次握手完成后请重新按下`;return}if(!pressedControlKeys.has(e.code)){pressedControlKeys.add(e.code);syncControlKeyState();updateSelectedGearFromHeldDirections();lastKeyboardEvent.textContent=`${keyNames[binding]}按下 · ${e.code}`;renderControlState();send().catch(console.error)}});
addEventListener('keyup',e=>{const binding=keys[e.code];if(!binding)return;if(!editingTarget(e.target))e.preventDefault();blockedControlKeys.delete(e.code);const wasPressed=pressedControlKeys.delete(e.code);syncControlKeyState();updateSelectedGearFromHeldDirections();lastKeyboardEvent.textContent=`${keyNames[binding]}释放 · ${e.code}`;renderControlState();if(wasPressed&&polling)send().catch(console.error)});
    ...
    channel.onmessage=async event=>{
      ...
      try{
        ...
        if(message.event==='vehicle_telemetry'){
          ...
          if(message.vcu_handshake)vcuHandshake={...vcuHandshake,adapter_ready:vcuAdapterReady(message.vcu_handshake,message.vehicle_adapter?.opened)};
          renderEstopRequest();
        ...
        vcuHandshake={...vcuHandshake,adapter_ready:vcuAdapterReady(nextVcuStatus,message.adapter_ready)};
