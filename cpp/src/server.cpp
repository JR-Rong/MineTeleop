...
<div class="workspace"><section class="visual-stage"><p id="estop-status" class="estop-banner" hidden>急停已触发；车辆必须本地确认后才能复位。</p><section id="cameras" class="grid"><div id="empty-stage" class="empty-stage">登录并连接车辆后显示实时视频</div></section><pre id="status" class="status-line">请先登录</pre></section>
...
function renderControlState(){const control=currentControl();for(const [name,element] of Object.entries(keyIndicators)){const active=Boolean(state[name]);element.classList.toggle('active',active);element.setAttribute('aria-pressed',String(active))}controlReadouts.gear.textContent=control.gear;controlReadouts.steering.textContent=control.steering.toFixed(2);controlReadouts.throttle.textContent=control.throttle.toFixed(2);controlReadouts.brake.textContent=control.brake.toFixed(2);document.getElementById('readout-steering').classList.toggle('active',Math.abs(control.steering)>0.001);document.getElementById('readout-throttle').classList.toggle('active',control.throttle>0.001);document.getElementById('readout-brake').classList.toggle('active',control.brake>0.001);const linkReady=polling&&peer?.connectionState==='connected'&&controlChannel?.readyState==='open',vcuReady=vcuHandshake.ready||(!vcuHandshake.supported&&vcuHandshake.state==='unsupported'),ready=linkReady&&vcuReady;inputReadiness.textContent=estopLatched?'急停锁定':(ready?'控制已就绪':(linkReady?'等待 VCU 握手':(polling?'等待控制链路':'等待连接')));inputReadiness.className=`status-chip ${estopLatched?'critical':(ready?'ok':'warn')}`}
...
async function logout(){closeRealtimeSession();controlAuthorityLost=true;webrtcLabel.textContent='正在释放控制权';await post('/api/disconnect',{reason:'driver_safe_logout'});authenticated=false;controlAuthorityLost=false;connectButton.textContent='连接所选车辆';renderAuthExpiry(0);sessionPanel.hidden=true;vcuPanel.hidden=true;monitorPanel.hidden=true;loginPanel.hidden=false;webrtcLabel.textContent='未连接';statusPanel.textContent=estopLatched?'已安全退出；车辆急停仍需本地确认复位':'已安全退出';clientLog('driver_safe_logout',{estop_latched:estopLatched})}
...
addEventListener('keydown',e=>{const binding=keys[e.code],estopKey=e.code==='KeyE';if(!binding&&!estopKey)return;if(editingTarget(e.target))return;e.preventDefault();if(!polling){lastKeyboardEvent.textContent=`${estopKey?'急停':keyNames[binding]}已截获 · 等待连接`;return}if(estopKey){if(!e.repeat){lastKeyboardEvent.textContent='急停已触发 · E';latchEstop('键盘 E');send({estop:true}).catch(console.error)}return}if(!state[binding]){state[binding]=true;lastKeyboardEvent.textContent=`${keyNames[binding]}按下 · ${e.code}`;renderControlState();send().catch(console.error)}});
