...
<div class="workspace"><section class="visual-stage"><p id="estop-status" class="estop-banner" hidden>急停已触发；车辆必须本地确认后才能复位。</p><section id="cameras" class="grid"><div id="empty-stage" class="empty-stage">登录并连接车辆后显示实时视频</div></section><section id="can-feedback-panel" class="can-feedback-panel" hidden aria-label="CAN 实时反馈"><div class="can-feedback-heading"><h2>CAN 实时反馈 · 测量值</h2><strong id="can-feedback-status" class="status-chip warn">等待车端遥测</strong></div><div class="can-summary"><div class="can-summary-item"><span>车速</span><strong id="can-speed">—</strong></div><div class="can-summary-item"><span>实际挡位</span><strong id="can-gear">—</strong></div><div class="can-summary-item"><span>物理选择器</span><strong id="can-selector">—</strong></div><div class="can-summary-item"><span>电子驻车 1-4</span><strong id="can-epb">—</strong></div><div class="can-summary-item"><span>VCU 状态</span><strong id="can-handshake">—</strong></div><div class="can-summary-item"><span>紧急开关</span><strong id="can-emergency">—</strong></div><div class="can-summary-item"><span>反馈时效</span><strong id="can-age">—</strong></div></div><div class="can-feedback-body"><div id="wheel-feedback-grid" class="wheel-feedback-grid" aria-label="八路轮端反馈"></div><div id="steering-feedback-grid" class="steering-feedback-grid" aria-label="四路转向反馈"></div></div></section><pre id="status" class="status-line">请先登录</pre></section>
...
function renderCanFeedback(){
  ...
  canSpeed.textContent=supported&&feedback.speed_valid?`${Number(feedback.speed_mps).toFixed(2)} m/s · ${(Number(feedback.speed_mps)*3.6).toFixed(1)} km/h`:'—';
  canGear.textContent=supported&&feedback.gear_valid?vcuGearLabel(feedback.gear):'—';
...
async function logout(){closeRealtimeSession();controlAuthorityLost=true;webrtcLabel.textContent='正在释放控制权';await post('/api/disconnect',{reason:'driver_safe_logout'});authenticated=false;controlAuthorityLost=false;connectButton.textContent='连接所选车辆';renderAuthExpiry(0);sessionPanel.hidden=true;vcuPanel.hidden=true;canFeedbackPanel.hidden=true;monitorPanel.hidden=true;loginPanel.hidden=false;webrtcLabel.textContent='未连接';statusPanel.textContent=estopLatched?'已安全退出；车辆急停仍需本地确认复位':'已安全退出';clientLog('driver_safe_logout',{estop_latched:estopLatched})}
    ...
    channel.onmessage=event=>{
      ...
      try{
        ...
        if(message.event==='vehicle_telemetry'){
          ...
          if(message.vcu_handshake)vcuHandshake={...message.vcu_handshake,driver_connected:true};
          renderMonitoring();
          return;
        }
