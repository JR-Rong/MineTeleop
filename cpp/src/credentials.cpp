...
[[nodiscard]] bool decimal_parameter(std::string_view text, std::string_view name,
                                     std::uint32_t* value) {
...
void require_argon2id() {
  if (!supports_argon2id()) {
    throw std::runtime_error(
        "Argon2id support is not compiled into this target; install the packaged Argon2 "
        "development dependency");
  }
}

[[nodiscard]] auto policy_key(const AuthenticationCostPolicy& policy) {
  return std::tie(policy.memory_kib, policy.time_cost, policy.parallelism, policy.hash_length,
                  policy.minimum_memory_kib, policy.maximum_memory_kib, policy.minimum_time_cost,
                  policy.maximum_time_cost, policy.minimum_parallelism, policy.maximum_parallelism,
                  policy.maximum_encoded_bytes);
}

struct AuthenticationCostPolicyLess {
  [[nodiscard]] bool operator()(const AuthenticationCostPolicy& left,
...
bool validate_authentication_cost_policy(const AuthenticationCostPolicy& policy,
                                         std::string* reason) {
  ...
  if (policy.memory_kib < policy.minimum_memory_kib ||
      ...
      policy.time_cost < policy.minimum_time_cost || policy.time_cost > policy.maximum_time_cost ||
  ...
  if (policy.maximum_encoded_bytes < 96 || policy.maximum_encoded_bytes > 4096) {
    set_reason(reason,
               "authentication cost policy encoded PHC budget must be between 96 and 4096 bytes");
    return false;
  }
#if MINE_TELEOP_HAS_ARGON2ID
  const auto encoded_length = argon2_encodedlen(
      policy.time_cost, policy.memory_kib, policy.parallelism, 16, policy.hash_length, Argon2_id);
...
bool constant_time_equal(std::string_view expected, std::string_view actual) noexcept {
  ...
  for (std::size_t index = 0; index < expected.size(); ++index) {
    difference |=
        static_cast<unsigned char>(expected[index]) ^ static_cast<unsigned char>(actual[index]);
  }
  return difference == 0;
#endif
}

bool validate_argon2id_verifier(std::string_view encoded, const AuthenticationCostPolicy& policy,
                                std::string* reason) {
  ...
  while (begin <= encoded.size()) {
    ...
    fields.push_back(encoded.substr(
        begin, separator == std::string_view::npos ? std::string_view::npos : separator - begin));
    if (separator == std::string_view::npos)
      break;
    begin = separator + 1;
  }
  if (fields.size() != 6 || fields[0] != "" || fields[1] != "argon2id" || fields[2] != "v=19") {
  ...
  const auto second =
      first == std::string_view::npos ? std::string_view::npos : params.find(',', first + 1);
...
std::string hash_argon2id_password(std::string_view password, std::span<const std::uint8_t> salt,
  ...
  const auto encoded_length =
      argon2_encodedlen(policy.time_cost, policy.memory_kib, policy.parallelism, salt.size(),
                        policy.hash_length, Argon2_id);
  if (encoded_length == 0 || encoded_length > policy.maximum_encoded_bytes + 1) {
  ...
  const int result = argon2id_hash_encoded(
      policy.time_cost, policy.memory_kib, policy.parallelism, password.data(), password.size(),
      salt.data(), salt.size(), policy.hash_length, encoded.data(), encoded.size());
  if (result != ARGON2_OK) {
    throw std::runtime_error(std::string("Argon2id password hashing failed: ") +
...
bool verify_argon2id_password(std::string_view encoded, std::string_view password,
                              const AuthenticationCostPolicy& policy) {
  require_argon2id();
  std::string reason;
  if (!validate_argon2id_verifier(encoded, policy, &reason)) {
  ...
  const int result = argon2id_verify(encoded_copy.c_str(), password.data(), password.size());
  if (result == ARGON2_OK)
    return true;
  if (result == ARGON2_VERIFY_MISMATCH)
    return false;
  throw std::runtime_error(std::string("Argon2id password verification failed: ") +
                           argon2_error_message(result));
#else
...
const std::string& dummy_argon2id_verifier(const AuthenticationCostPolicy& policy) {
  ...
  static std::map<AuthenticationCostPolicy, std::string, AuthenticationCostPolicyLess> cache;
  {
    ...
    if (const auto found = cache.find(policy); found != cache.end())
      return found->second;
  }
  constexpr std::array<std::uint8_t, 16> salt{0x6d, 0x69, 0x6e, 0x65, 0x2d, 0x74, 0x65, 0x6c,
                                              0x65, 0x6f, 0x70, 0x2d, 0x64, 0x75, 0x6d, 0x6d};
  auto verifier = hash_argon2id_password("mine-teleop-unknown-driver-verifier", salt, policy);
  std::lock_guard lock(mutex);
