...
void automatic_driver_runtime() {
  for (const auto& outcome : {"approve", "reject", "timeout", "logout", "end", "replace"}) {
    ...
    // The console can select a driver other than its configured default.
    check(driver.login("driver-secret", "d2").at("driver_id") == "d2", "selected driver login failed");
    ...
    const auto inbox = http.get_json(base + "/mobile/api/requests",
        {{"X-Mine-Teleop-Approver-Token", token}}).at("requests");
    check(inbox.size() == 1 && inbox.at(0).at("driver_id") == "d2", "approval used configured default driver");
