...
void test_disarm_waits_for_torque_stop_neutral_park_and_manual() {
  ...
  const auto& parking_brake_hold = find_frame(frames, mine_teleop::vcu::ids::kAduEhb01);
  expect(signal(parking_brake_hold, 0, 4) == 1 && signal(parking_brake_hold, 4, 12) == 250,
