...
Json read_json(const std::filesystem::path& path) {
  ...
  if (!input) throw TestFailure("cannot open protocol contract input: " + path.string());
  try {
    return Json::parse(input);
  } catch (const Json::exception& error) {
    throw TestFailure("cannot parse protocol contract input " + path.string() + ": " + error.what());
  }
}
...
void validate_schema_shape(const Json& schema) {
  const std::set<std::string> root_keywords{
      "$schema", "$id", "title", "description", "$comment", "type", "required", "properties",
      "additionalProperties"};
  const std::set<std::string> property_keywords{
      "title", "description", "$comment", "type", "const", "enum", "minimum", "maximum",
      "minLength", "default"};
  ...
  for (const auto& [key, ignored] : schema.items()) {
    ...
    expect(
        root_keywords.contains(key) || is_extension_keyword(key),
  ...
  expect(schema.contains("required") && schema.at("required").is_array(), "schema required must be an array");
  expect(
  ...
  for (const auto& required : schema.at("required")) {
    ...
    expect(
        properties.contains(required.get<std::string>()),
  ...
  for (const auto& [name, property] : properties.items()) {
    ...
    expect(property.contains("type") && property.at("type").is_string(), "schema property has no type: " + name);
    for (const auto& [key, ignored] : property.items()) {
      static_cast<void>(ignored);
      expect(
          property_keywords.contains(key) || is_extension_keyword(key),
...
[[nodiscard]] std::optional<std::string> validate_property(
    const Json& property,
    const Json& value,
    std::string_view field) {
  const auto type = property.at("type").get<std::string>();
  if (!matches_type(value, type)) return "wrong type for " + std::string(field);
  ...
  if (property.contains("enum")) {
    ...
    if (!matched) return "value outside enum for " + std::string(field);
  }
...
[[nodiscard]] std::optional<std::string> validate_message(const Json& schema, const Json& value) {
  if (!value.is_object()) return "message is not an object";
  const auto& properties = schema.at("properties");
  for (const auto& required : schema.at("required")) {
    const auto field = required.get<std::string>();
    if (!value.contains(field)) return "missing required field: " + field;
  }
  for (const auto& [field, field_value] : value.items()) {
    if (!properties.contains(field)) {
      if (!schema.at("additionalProperties").get<bool>()) return "unknown field: " + field;
      continue;
    }
    if (const auto error = validate_property(properties.at(field), field_value, field)) return error;
  }
  return std::nullopt;
}
...
std::vector<ContractVector> read_vectors(
    const Json& manifest,
    const std::filesystem::path& manifest_path) {
  expect(
      manifest.value("format", "") == "mine-teleop-control-command-contract-v1",
      "unexpected protocol contract manifest format");
  expect(manifest.contains("vectors") && manifest.at("vectors").is_array(), "manifest vectors must be an array");
  ...
  for (const auto& entry : manifest.at("vectors")) {
    ...
    expect(
        entry.contains("structural_valid") && entry.at("structural_valid").is_boolean(),
    ...
    vectors.push_back({entry.at("name").get<std::string>(), entry.at("structural_valid").get<bool>(), std::move(value)});
  }
  return vectors;
}
...
void run_contract_tests() {
  ...
  const auto manifest_path = std::filesystem::path("protocol/v1/fixtures/control-command-vectors.json");
  const auto manifest = read_json(manifest_path);
  expect(manifest.contains("schema") && manifest.at("schema").is_string(), "manifest schema path is missing");
  const auto schema = read_json(manifest_path.parent_path() / manifest.at("schema").get<std::string>());
  validate_schema_shape(schema);
  expect(
      schema.at("x-portable-json-integer-maximum").get<std::uint64_t>() == kPortableJsonIntegerMaximum,
      "schema portable JSON integer maximum changed");
  const auto& schema_properties = schema.at("properties");
  expect(
      schema_properties.at("seq").at("maximum").get<std::uint64_t>() == kPortableJsonIntegerMaximum &&
  ...
  expect(
      manifest.at("numeric_compatibility").at("portable_json_integer_maximum").get<std::uint64_t>() ==
          kPortableJsonIntegerMaximum,
  ...
  for (const auto& vector : vectors) {
    ...
    expect(
        structural_valid == vector.structural_valid,
        "schema result disagrees with vector " + vector.name +
            (structural_error ? ": " + *structural_error : ""));
    if (!structural_valid) continue;

    ControlCommand command;
    try {
      command = ControlCommand::from_json(vector.value);
    } catch (const std::exception& error) {
      throw TestFailure("schema-valid vector was rejected by ControlCommand::from_json: " + vector.name + ": " + error.what());
    }
    if (vector.value.contains("future_extension")) {
      expect(
          !command.to_json().contains("future_extension"),
