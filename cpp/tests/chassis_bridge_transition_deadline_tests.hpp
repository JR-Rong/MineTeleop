...
void test_transition_deadline_supervisor_boundaries() {
  ...
  const auto deadline_for = [](State state) {
    ...
    expect(
        mine_teleop_chassis_test_transition_deadline_ms(
            static_cast<int>(state), &deadline_ms) == 1 &&
            deadline_ms > 0,
        "transition supervisor did not expose a deadline for a monitored state");
    return deadline_ms;
  };
  const auto observe = [](State state,
                          std::uint64_t epoch,
                          std::int64_t now_ms,
                          std::int64_t* elapsed_ms = nullptr,
                          std::int64_t* limit_ms = nullptr) {
    ...
    const int result = mine_teleop_chassis_test_transition_deadline_observe(
        ...
        epoch,
        now_ms,
        elapsed_ms == nullptr ? &local_elapsed : elapsed_ms,
  ...
  expect(
      wait_handshake_limit == 4000 && wait_epb_limit == 5000 &&
          wait_gear_limit == 3000 && wait_modes_limit == 4000 &&
          torque_limit == 3000 && stop_limit == 10000 &&
  ...
  expect(
      wait_gear_limit != 500 && stop_limit > wait_gear_limit,
  ...
  expect(
      mine_teleop_chassis_test_transition_deadline_reset() == 0 &&
          ...
          observe(
              State::WaitGear,
              44,
              replacement_entry + wait_gear_limit - 1) == 0 &&
          observe(
              State::WaitGear,
              44,
              replacement_entry + wait_gear_limit) == 1,
  ...
  expect(
      mine_teleop_chassis_test_transition_deadline_ms(
          static_cast<int>(State::Ready), &ignored_deadline) == 0 &&
          mine_teleop_chassis_test_transition_deadline_reset() == 0 &&
...
void test_wait_gear_transition_timeout_preserves_staged_disarm() {
  ...
  bool opened = false;
  const auto cleanup = [&] {
    if (opened) static_cast<void>(mine_teleop_chassis_close());
    ...
    if (transport[0] >= 0) ::close(transport[0]);
    if (transport[1] >= 0) ::close(transport[1]);
  };
  ...
  try {
    ...
    expect(
        mine_teleop_chassis_open_v2(&config) == 0,
    ...
    expect(
        mine_teleop_chassis_apply_state(
            3, 5.0, 0.0, steering.data(), steering.size()) == 0,
    ...
    expect_all_brake_pressure_raw(
        stop_frames,
        4095,
        "transition timeout did not preserve EHB safety braking during disarm");
  ...
  expect(
      timed_out->value("stage", "") == "vcu_transition_wait_gear" &&
          ...
          std::find(
              timed_out->at("missing_or_mismatched").begin(),
