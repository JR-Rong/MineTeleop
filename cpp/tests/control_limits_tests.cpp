...
static_assert(limits::kMaxFullScaleMotorTorqueNm ==
              MINE_TELEOP_CHASSIS_MAX_FULL_SCALE_MOTOR_TORQUE_NM);
static_assert(limits::kMaxOrdinaryBrakePressureBar ==
              MINE_TELEOP_CHASSIS_MAX_ORDINARY_BRAKE_PRESSURE_BAR);
static_assert(limits::kBrakePressureResolutionBar ==
              MINE_TELEOP_CHASSIS_BRAKE_PRESSURE_RESOLUTION_BAR);
static_assert(limits::kMaxMotorTorqueRiseRateNmPerSecond ==
              MINE_TELEOP_CHASSIS_MAX_MOTOR_TORQUE_RISE_RATE_NM_PER_SECOND);
static_assert(limits::kMinSpeedFeedbackTimeoutMs ==
              MINE_TELEOP_CHASSIS_MIN_SPEED_FEEDBACK_TIMEOUT_MS);
static_assert(limits::kMaxSpeedFeedbackTimeoutMs ==
...
static_assert(limits::kMaxSpeedPidDerivativeFilterTauMs ==
              MINE_TELEOP_CHASSIS_MAX_DERIVATIVE_FILTER_TAU_MS);
static_assert(limits::kMaxHardOverspeedMarginMps ==
...
mine_teleop::SessionControlProfileRequest profile_request(
    std::uint64_t sequence, mine_teleop::SessionControlProfile profile) {
...
void test_units_and_axis_helpers() {
  expect_near(
      limits::meters_per_second_to_kilometers_per_hour(limits::kChassisControlMaxTargetSpeedMps),
      limits::kChassisControlMaxTargetSpeedKph, "m/s to km/h changed target-speed semantics");
  expect_near(
      limits::kilometers_per_hour_to_meters_per_second(limits::kChassisControlMaxTargetSpeedKph),
      limits::kChassisControlMaxTargetSpeedMps, "km/h to m/s changed target-speed semantics");
  expect_near(limits::steering_degrees_to_normalized_request(limits::kMaxSteeringAngleDeg),
              limits::kMaxNormalizedSteeringRequest,
              "maximum steering degrees did not map to the normalized maximum");
  expect_near(
      limits::normalized_steering_request_to_degrees(-limits::kMaxNormalizedSteeringRequest),
      -limits::kMaxSteeringAngleDeg, "negative normalized steering did not preserve direction");
  ...
  expect(limits::bounded_steering_axis_count(-1) == 0U &&
...
void test_strict_finite_boundaries_and_quantization() {
  ...
  expect(!limits::is_finite_inclusive(
             std::nextafter(maximum, std::numeric_limits<double>::infinity()), 0.0, maximum),
         "next representable value above a hard cap was accepted");
  expect(!limits::is_finite_inclusive(std::nextafter(0.0, -std::numeric_limits<double>::infinity()),
                                      0.0, maximum),
         "negative adjacent value was accepted");
  expect(!limits::is_finite_inclusive(std::numeric_limits<double>::quiet_NaN(), 0.0, maximum) &&
             !limits::is_finite_inclusive(std::numeric_limits<double>::infinity(), 0.0, maximum) &&
             !limits::is_finite_inclusive(-std::numeric_limits<double>::infinity(), 0.0, maximum),
  ...
  for (const auto pressure : pressures) {
    const double cpp_quantized = limits::quantize_ordinary_brake_pressure_bar_toward_zero(pressure);
    expect(cpp_quantized == mine_teleop_chassis_quantize_brake_pressure_bar(pressure),
           "C++ and C ABI brake pressure quantization drifted");
    expect(cpp_quantized >= 0.0 && cpp_quantized <= maximum,
           "brake pressure quantization escaped the ordinary pressure range");
    if (std::isfinite(pressure) && pressure > 0.0 && pressure <= maximum) {
      expect(cpp_quantized <= pressure, "brake pressure quantization rounded away from zero");
    }
  }
  expect_near(limits::quantize_ordinary_brake_pressure_bar_toward_zero(10.19), 10.1,
              "brake pressure quantization did not floor to the DBC resolution");

  MineTeleopChassisSpeedPidConfig pid{
      MINE_TELEOP_CHASSIS_MAX_SPEED_PID_GAIN, MINE_TELEOP_CHASSIS_MAX_SPEED_PID_GAIN,
      MINE_TELEOP_CHASSIS_MAX_SPEED_PID_GAIN, MINE_TELEOP_CHASSIS_MAX_DERIVATIVE_FILTER_TAU_MS,
      MINE_TELEOP_CHASSIS_MAX_SPEED_PID_MAX_DT_MS};
  expect(mine_teleop_chassis_speed_pid_config_is_valid(&pid) != 0,
         "C ABI accepted cap was rejected");
  pid.kp = std::nextafter(MINE_TELEOP_CHASSIS_MAX_SPEED_PID_GAIN,
                          std::numeric_limits<double>::infinity());
  expect(mine_teleop_chassis_speed_pid_config_is_valid(&pid) == 0,
...
void test_protocol_and_vehicle_limits_are_independent() {
  ...
  global_cap.validate();
  global_cap.target_speed_kph = std::nextafter(limits::kChassisControlMaxTargetSpeedKph,
                                               std::numeric_limits<double>::infinity());
  expect_throws([&] { global_cap.validate(); },
  ...
  for (const double invalid_value :
    ...
    expect_throws([&] { profile.validate(); },
                  "protocol profile accepted a non-finite or negative target speed");
  }

  mine_teleop::VehicleControlService service(vehicle_config(), "driver-r10", "session-r10",
                                             "r10-control-token",
                                             std::make_unique<mine_teleop::MockVehicleAdapter>());
  const auto now =
      test::clock_sample(mine_teleop::UtcMillis{1'000}, mine_teleop::MonotonicMillis{1'000});
  service.start(now);
  ...
  const std::array<VehicleLimitCase, 4> cases{{
      {"target_speed_exceeds_vehicle_limit",
       [](mine_teleop::SessionControlProfile& profile) {
         profile.target_speed_kph = std::nextafter(10.0, std::numeric_limits<double>::infinity());
       }},
      {"motor_torque_exceeds_vehicle_limit",
       [](mine_teleop::SessionControlProfile& profile) {
         profile.max_motor_torque_nm =
             std::nextafter(100.0, std::numeric_limits<double>::infinity());
       }},
      ...
      {"steering_exceeds_vehicle_limit",
       [](mine_teleop::SessionControlProfile& profile) {
         profile.max_steering_angle_deg =
             std::nextafter(20.0, std::numeric_limits<double>::infinity());
       }},
  }};
  ...
  for (const auto& item : cases) {
    ...
    profile.validate();
    const auto result =
        service.receive_session_profile(profile_request(sequence++, std::move(profile)), now);
    expect(!result.accepted && result.reason == item.expected_reason,
           std::string("vehicle layer did not independently reject ") + item.expected_reason);
  }
  service.close();
}
